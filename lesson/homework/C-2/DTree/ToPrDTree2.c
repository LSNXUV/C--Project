...
//根据前序遍历和中序遍历构建二叉树
DBTree* CreateBiTree(char *instr, char *prestr, int length)
{
    if (length == 0)//都构建完毕
    {
        return NULL;
    ...
    T->lchild = NULL;
    T->rchild = NULL;
    T->lchild = CreateBiTree(&instr[0], &prestr[1], leftlength);//创建左子树
    T->rchild = CreateBiTree(&instr[i + 1], &prestr[i+1], rightlength);//创建右子树
...
int main()
{
    ...
    DBTree *T;
    T = (DBTree*)malloc(sizeof(DBTree));
    T = CreateBiTree(instr, prestr, length);
    AftOrderTraverse(T);
    // scanf("%d",length);
    return 0;
}